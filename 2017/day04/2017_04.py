inputs = '''kvvfl kvvfl olud wjqsqa olud frc
...
#Puzzle 1
valid_count = 0
for phrase in inputs.split("\n"):

    phrase_tokens = phrase.split()
    if len(phrase_tokens) == len(set(phrase_tokens)):
        valid_count += 1
...
for phrase in inputs.split("\n"):
    ...
    invalid = False
    phrase_tokens = phrase.split()
    sorted_tokens = [''.join(sorted(token)) for token in phrase_tokens]
    token_count = len(sorted_tokens)
    for index_one in range(token_count-1):
        for index_two in range(index_one+1, token_count):
            if is_anagram(sorted_tokens[index_one], sorted_tokens[index_two]):
                invalid = True
                break
        if invalid:
            break
